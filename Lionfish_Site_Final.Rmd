...
qqPlot(site$per.h)
model_hectare=glm(formula=per.h~ depth + site + year, family = Gamma(link=log), data=site)
summary(model_hectare)
```

```{r}
shapiro.test(site$per.q)
qqPlot(site$per.q)
model_quad=glm(formula=per.q~ depth + site + year, family = Gamma(link=log), data=site)
summary(model_quad)
```
