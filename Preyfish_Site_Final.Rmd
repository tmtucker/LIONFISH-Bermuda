...
```
```{r}
trophic_biomass_depth <-ggplot(data = mean_trophic, aes(x = site, y=total_bio, fill = trophic_guild)) + geom_bar(stat = "summary", fun.y = "mean", position = "stack", color = "black") + labs(y = "Mean % Contribution by Biomass", x = "Site") + scale_fill_brewer(palette = "YlGnBu", labels = c("Herbivore", "Invertivore", "Macrocarnivore", "Omnivore", "Planktivore"), name = ("Trophic Guild")) +theme_bw() + theme(legend.title = element_text(size = 14), legend.text = element_text(size = 12)) + theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14)) + facet_grid(year~.)
trophic_biomass_depth
ggsave(filename = "trophic_biomass_depth.eps",plot = trophic_biomass_depth, device = "eps",width = 8,height = 4,dpi = 600)
```
...
ggsave(filename = "biomass_site_depth_year.eps",plot = biomass_site_depth_year, device = "eps",width = 6,height = 4,dpi = 600)
```
